...
const InfoRow = ({ icon: Icon, label, value }: { icon: any; label: string; value: string }) => (
  <div className="flex items-start gap-4 p-4 rounded-2xl bg-brand-bg border border-ocl-primary/5">
    ...
    <div className="min-w-0">
      ...
      <p className="text-sm font-bold text-ocl-primary break-words">{value || '—'}</p>
    </div>
  </div>
);
        ...
        >
          ...
          >
            ...
            <div className="px-8 pb-8 space-y-3">
              ...
              <button
                onClick={onClose}
                className="w-full mt-4 py-3.5 border border-ocl-primary/10 rounded-2xl font-black text-xs uppercase tracking-widest text-brand-text/40 hover:bg-brand-bg transition-all"
              >
                Fechar
              </button>
            </div>
