...
export const BookingFormModal = ({ isOpen, selectedDate, selectedSlot, onClose, onConfirm }: BookingFormModalProps) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Reset on success
      setForm({ contract: '', phone: '', responsible_type: 'Titular', responsible_name: '', agreed_values: '' });
      setErrors({});
    } finally {
      setLoading(false);
    }
    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          >
            {/* Header accent bar */}
            <div className="h-1.5 w-full bg-gradient-to-r from-ocl-primary via-brand-accent to-ocl-primary" />
            ...
            <div className="p-8 space-y-5">
              ...
              <div>
                <label className="text-[10px] font-black text-brand-text/40 uppercase tracking-widest flex items-center gap-1.5 mb-2">
                  <DollarSign className="w-3 h-3" /> Valores Acordados (R$)
                </label>
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-sm font-black text-brand-text/30">R$</span>
                  <input
                    id="booking-values"
                    type="text"
                    inputMode="numeric"
                    value={form.agreed_values}
                    onChange={e => { setForm(f => ({ ...f, agreed_values: formatCurrency(e.target.value) })); setErrors(er => ({ ...er, agreed_values: undefined })); }}
                    placeholder="0,00"
                    className={`w-full bg-brand-bg border rounded-2xl pl-10 pr-5 py-3.5 text-sm font-bold text-ocl-primary placeholder:font-normal placeholder:text-brand-text/30 focus:ring-4 focus:ring-brand-accent/10 focus:border-brand-accent outline-none transition-all ${errors.agreed_values ? 'border-brand-danger' : 'border-ocl-primary/10'}`}
                  />
                </div>
